...
if [ $# -ne 3 ]; then
    echo "Usage: $0 file_path_out file_path_fasta number_of_hits_to_keep > new_fasta.fasta"
    exit 1
fi
...
# Check if the # of hits the user wants to save is higher than the # of sequences in the fasta file
if [ $num_of_seqs -lt $number_of_hits ]; then
        echo "Error: the number of hits to keep is greater than the total number of sequences"
    exit 1
fi
